import React, { useState } from 'react'
import "../../Style/css/UpcomingCourses.css"
import { useSelector } from 'react-redux';
import { FiUser, FiClock,FiBook } from "react-icons/fi";
import { FaArrowLeft, FaArrowRight } from "react-icons/fa";
const UpcomingCourses2 = () => {
  const {upcomingCourses} = useSelector((store) => store.edu);
  const [firstIndex, setFirstIndex] = useState(0);
  const [lastIndex, setLastIndex] = useState(2);
  const coursesList = upcomingCourses.slice( firstIndex, lastIndex);
  const nextElement = () => {
    setFirstIndex((firstIndex) => {
      let newIndex = firstIndex + 1;
      if(newIndex > 5) {
        return newIndex = firstIndex
      }
      return newIndex
    })
    setLastIndex((lastIndex) => {
      let newIndex = lastIndex + 1;
      if(newIndex > upcomingCourses.length) {
        return newIndex = upcomingCourses.length
      }
      return newIndex
    })
  }
  const prevElement = () => {
    setFirstIndex((firstIndex) => {
      let newIndex = firstIndex - 1;
      if(newIndex < 0) {
        return newIndex = 0
      }
      return newIndex
    })
    setLastIndex((lastIndex) => {
      let newIndex = lastIndex - 1;
      if(newIndex < 2) {
        return newIndex = 2
      }
      return newIndex
    })
  }
  return (
    <div className='upcoming_courses_middle'>
      <header>
        <h1>Our Upcoming<br/>
        Courses
        </h1>
        <div className="control_buttons">
          <button onClick={prevElement}><FaArrowLeft/></button>
          <button onClick={nextElement}><FaArrowRight/></button>
        </div>
      </header>
      <main className='upcomingCourses_list_middle'>
        {
          coursesList.map((item) => {
            const {id, img, title, type, studentsAmount, startDate, lessonsAmount} = item;
            return (
              <article className='card' key={id}>
                <div className="img_box ">
                  <img className='card-img-top' src={img} alt={title} />
                </div>
                <div className="card-body">
                  <h1 className="title">{title}</h1>
                  <p className="lecturer">Lecturer <span className='admin'>admin</span> in <span className='type'>{type}</span> </p>
                  <hr />

                  <div className="details">
                    <div className="studentsAmount"><span><FiUser/></span> {studentsAmount}</div>
                    <div className="startDate"><span><FiClock/></span>{startDate}</div>
                    <div className="lessonsAmount"><span><FiBook/></span>{lessonsAmount} Lessons</div>
                  </div>
                </div>
              </article>
            )
          })
        }
      </main>
    </div>
  )
}

export default UpcomingCourses2